...
7.  [Notify BigPicture Sweden](#notify-bigpicture-sweden)
...
1.  Copy `sda-cli` and `crypt4gh_bp_key.pub` to the wharf
    ...
    Copy the `crypt4gh_bp_key.pub` file to the project drive in the wharf:

    ``` bash
    cp crypt4gh_bp_key.pub ~/<project_name>/
    ```

2.  Copy `sda-cli` and `crypt4gh_bp_key.pub` from the wharf to Bianca
    ...
    cp /proj/[project_name]/nobackup/wharf/[username]/[username]-[project_name]/crypt4gh_bp_key.pub .
    ```

    Running the `ls` command in this folder, you should be able to see
    the `sda-cli` and `crypt4gh_bp_key.pub` files.
:::
...
## Notify BigPicture Sweden {#notify-bigpicture-sweden}

Once your submission is completed, please don't forget to notify us by sending an email to [BigPicture Sweden Helpdesk](mailto:{{< var email.helpdesk >}}). The email should contain
