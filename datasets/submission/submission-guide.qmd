...
1.  Obtain the configuration file
    ...
    Before uploading, you'll need to download your configuration file. Just log in [here](https://login.bp.nbis.se/) and follow the prompts to authenticate. Once logged in, click **Download credentials to upload to the Inbox** to get your `s3cmd-inbox.conf` file.  (Note: If you are performing a test upload, please use our [staging service](https://staging-login.bp.nbis.se/) instead.)

2.  Upload the files

    Files can be uploaded individually or in groups. By default, the local folder structure is not preserved on the remote storage; all files will be placed in the root of your inbox.

    To upload files into a specific remote folder, use the `--target-directory <path>` flag.
    ...
    In many cases, it is more efficient to upload an entire directory. By using the recursive  `-r` flag, the local folder structure will be preserved on the remote server.
